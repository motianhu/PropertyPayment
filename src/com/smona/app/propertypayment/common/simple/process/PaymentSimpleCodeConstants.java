package com.smona.app.propertypayment.common.simple.process;

public class PaymentSimpleCodeConstants {
    public static final String MSG_CITY = "1400";
    public static final String MSG_CITY_RESPONSE = "1410";
    
    //power
    public static final String MSG_POWER_USER_INFO = "1200";
    public static final String MSG_POWER_SUBMIT = "1300";
    public static final String MSG_POWER_COMPANY = "1800";
    public static final String MSG_POWER_DETAIL = "1700";

    public static final String MSG_POWER_USER_INFO_RESPONSE = "1210";
    public static final String MSG_POWER_SUBMIT_RESPONSE = "1310";
    public static final String MSG_POWER_COMPANY_RESPONSE = "1810";
    public static final String MSG_POWER_DETAIL_RESPONSE = "1710";
    
    //water
    public static final String MSG_WATER_USER_INFO = "2800";
    public static final String MSG_WATER_SUBMIT = "2900";
    public static final String MSG_WATER_COMPANY = "3000";
    public static final String MSG_WATER_DETAIL = "3400";

    public static final String MSG_WATER_USER_INFO_RESPONSE = "2810";
    public static final String MSG_WATER_SUBMIT_RESPONSE = "2910";
    public static final String MSG_WATER_COMPANY_RESPONSE = "3010";
    public static final String MSG_WATER_DETAIL_RESPONSE = "3410";
    
    //gas
    public static final String MSG_GAS_USER_INFO = "3200";
    public static final String MSG_GAS_SUBMIT = "3300";
    public static final String MSG_GAS_COMPANY = "2600";
    public static final String MSG_GAS_DETAIL = "3500";

    public static final String MSG_GAS_USER_INFO_RESPONSE = "3210";
    public static final String MSG_GAS_SUBMIT_RESPONSE = "3310";
    public static final String MSG_GAS_COMPANY_RESPONSE = "2610";
    public static final String MSG_GAS_DETAIL_RESPONSE = "3510";
    
    //heat
    public static final String MSG_HEAT_USER_INFO = "2000";
    public static final String MSG_HEAT_SUBMIT = "2100";
    public static final String MSG_HEAT_COMPANY = "1900";
    public static final String MSG_HEAT_DETAIL = "3600";
    public static final String MSG_HEAT_QIANFEI_DETAIL = "2400";

    public static final String MSG_HEAT_USER_INFO_RESPONSE = "2010";
    public static final String MSG_HEAT_SUBMIT_RESPONSE = "2110";
    public static final String MSG_HEAT_COMPANY_RESPONSE = "1910";
    public static final String MSG_HEAT_DETAIL_RESPONSE = "3610";
    public static final String MSG_HEAT_QIANFEI_DETAIL_RESPONSE = "2410";
}
